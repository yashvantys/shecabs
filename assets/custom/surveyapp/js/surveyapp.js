...
*/
		...
		*/
        ...
        //set Intelligent survey - check publish links available or not
        var selectedPublishLinkOptionId ='';
        if($(".render-type-publishlinks").length > 0){
        	$(".review-text").parent('.question').addClass('rule-validate-failed');
        	selectedPublishLinkOptionId = $(".source-publish-link-default input:hidden").attr('value');
        }
        var selectedAvatar = 0;
        var publishLinkEnabled = 0;
        var publishLinkQuestionExist = 0;
        var getPublishAvatar = '';
        // Getting publish links when click physcian or location radio
		$("div.physician_location_radio input").on("click",function(){
			getPublishAvatar = $(this).val();
            if($(".render-type-promoter_radio input:radio").length > 0 && $("body .render-type-promoter_radio input:radio").is(":checked")){
            	getPublishLink(getPublishAvatar);
			}
			
		});
		// Getting publish links 
		$("div.render-type-promoter_radio input").on("click",function(){
			getPublishLink(getPublishAvatar);
		});
		
		// Auto submit survey form when click publish link 
        $("body").on("click", "input.publish-source-radio-check",function(e){
//        	e.preventDefault();
        	var radioName = $(this).attr("name");
        	var radioValue = $(this).val();
        	var publishUrl = $(this).attr('publish-url');
        	var selectedRadioObj = document.createElement('input');
        	selectedRadioObj.setAttribute('type', 'hidden');
        	selectedRadioObj.setAttribute('name', radioName);
        	selectedRadioObj.setAttribute('id', 'redirect_url');
        	selectedRadioObj.setAttribute('value',radioValue );
        	selectedRadioObj.setAttribute('publish-url',publishUrl );
        	$('form#survey-form').append(selectedRadioObj);
        	$(".source-publish-link-default .source-link-hidden").val("");
        	$('.submit-survey').trigger('click');
        });
        
		$('#survey-form').submit(function(e){
			e.preventDefault();
			var publishLinkSubmit = 0;
			var me = $(this);
			 var parentObj = $("button[type='submit'].submit-survey").parents(".survey-page");
             // validate the current page here start
             var validated = validate_current_page(parentObj);
             if($("body .publish-link-radio-check").length > 0){
            	 publishLinkSubmit = 1; 
             }
             //  $('#survey-form').validate({});
             if(validated){
             ...
             if(validated) {
            	 ...
            	 // Selected Publish link redirect to new tab
            	 if(publishLinkSubmit == 1) {
//            		 validatePromoterQuestion =1;
                	   var selected = $('#redirect_url');
                	   var sourceUrl = selected.attr('publish-url');
                	   console.log(sourceUrl);
                	   sourceLinkLength = $('.rule-validate-success .render-type-publishlinks').length;
                	   if(sourceUrl){
                		   window.open(sourceUrl,'mywindow').focus()
...
//             }
        ...
        // Getting publish links based on promoter question option and selected physician/ location
        function getPublishLink(selectAvatar){
        	var selectedAvatar =selectAvatar;
        	var avatarArray = [];
        	if(selectAvatar !=""){
        		selectedAvatar =selectAvatar;
        		avatarArray.push(selectAvatar);
        	}
        	if(SocialView.avatarId !=""){
				selectedAvatar = SocialView.avatarId;
				avatarArray.push(SocialView.avatarId);
			}
			if($(".physician_location_radio input:radio").length > 0 && $("body .physician_location_radio input:radio").is(":checked")){
				selectedAvatar = selectAvatar;
			}
			if(selectedAvatar >= 0){
				var productId ='';
				set_question_numbers();
				selectedPublishLinkOptionId = $(".source-publish-link-default input:hidden").attr('source-link-option-id');	
				if(SocialView.productId!=""){
					productId= SocialView.productId;
				}
				$(".source-publishlinks").html("");
				Custom.showLoader(".survey-page");
				$(".blockUI").css("position","fixed");
				var url = SocialView.base_url + 'surveyapp/'+SocialView.client_id+'/'+SocialView.survey_mode+'/'+SocialView.avatarId+'/'+SocialView.survey_id+'/'+SocialView.language+'/get_avatar_publish_link'
				$.ajax({
					type:"GET",
					url: url,
					data : {selectedAvatar:avatarArray,optionId:selectedPublishLinkOptionId,productId:productId}

				}).done(function(resp){
					if(resp !=''){
						if($(".render-type-publishlinks").parent( ".question" ).hasClass('rule-validate-success')){
							$(".source-publishlinks").html(resp);
//			      				$(".render-type-publishlinks").parent( ".question" ).css('display','block');
							$('.options input:radio').jqTransRadio();
						}
						set_question_numbers();
					}else{
						$(".render-type-publishlinks").parent( ".question" ).removeClass('rule-validate-success');
		      			$(".render-type-publishlinks").parent( ".question" ).addClass('rule-validate-failed');
		      			set_question_numbers();
		      		}
					Custom.hideLoader(".survey-page");
				});
