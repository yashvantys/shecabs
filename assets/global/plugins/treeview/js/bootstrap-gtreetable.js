...
(function ($) {
    ...
    GTreeTable.prototype = {
        ...
        /*
         * Ajax Call to get Tree View data.
         * */
        getSourceNodes: function (nodeId, nodeLevel, force) {
            ...
            var sourceOptions = this.options.source(nodeId, nodeLevel);
            var defaultSourceOptions = {
                beforeSend: function () {
                	//If node id exits then rounding progress bar icon will be displayed.
                	//If it is an initial call then regular theme progress bar will be displayed.
                	if (nodeId > 0 || nodeId.length > 0) {
                        oNode.isLoading(true);
                    }
                	else
                	{
                		Custom.showLoader('#pgtreeviewdropdown .modal-body');
                	}
                },
                ...
                complete: function () {
                	//The below code is to hide the progress bar on completion of data retrieval.
                	if (nodeId > 0  || nodeId.length > 0) {
                        oNode.isLoading(false);
                    }
                    else
                	{
                		Custom.hideLoader('#pgtreeviewdropdown .modal-body');
                	}
                }
            };
        ...
        init: function () {
            ...
            this.getSourceNodes(-1, 0, 0).done(function (result) {
    ...
    function GTreeTableNode(data, gtreetable) {
        ...
        this.type = data.type;
        this.id = data.id;
        this.hasChildNodes = data.childNodes;
        this.ddid = data.ddid;
    ...
    GTreeTableNode.prototype = {
        ...
        render: function() {
            ...
            /*
             * While rendering the TreeView dropdown,  it checks for Physician Group Dropdown like 
             * whether any options are selected or not.  If any options are selected then those options
             * has to be selected in Physician Group Tree view too.
             * 
             * So for that it is getting the selected values from Physician Group Dropdown.
             * */
            var phyGroup = [];
            var phyIdGroup = [];
            phyGroup = $('#physicianGroup').val();
            
            if (this.id !== undefined) {
                this.$node.attr('data-id', this.id);
                
                /*
                 * Selecting the Options in Physician Group Tree View based on options
                 * selected in Physician Group dropdown.
                 * */
                if(phyGroup != null)
                {
                	for(var i=0;i<phyGroup.length;i++)
                	{
                		var pgidArray = phyGroup[i].split("|");
                		if(pgidArray[0] != null)
                		{
                			if(this.id == pgidArray[0])
                            {
            ...
            //If a node has no childnodes then that node to be displayed with out left arrow and with correct allignment
            //For that, if a node has no childnodes then in such case the margin-left is increased to 20px.
            var indentForArrow = 0;
            
            if(this.hasChildNodes == false)
            {
            	indentForArrow = 20;
            	//If a node has no child nodes then left arrow should not be displayed.
            	//and that can be done by removing the class "node-icon-ce"
            	this.$ceIcon.removeClass("node-icon-ce");
            }
            
            this.$indent.css('marginLeft', (((parseInt(this.level) - this.manager.options.rootLevel) * this.manager.options.nodeIndent) + indentForArrow) + 'px').html('&zwnj;');
        ...
        expand: function (options) {
            ...
            $.when(this.manager.getSourceNodes(oNode.id, parseInt(oNode.level), settings.isAltPressed)).done(function (result) {
