...
class Form_builder {
    ...
    function __construct($config = array()) {
        ...
        /*
            Below lines has been added to Customize the library
            so that it supports Form Client Side Validation.
        */
    ...
    function open_form($options) {
        ...
        {
        	$this->config['default_control_label_class'] = $options['label_class'];
        }
        
        if(isset($options['form_control_class']))
        {
        	$this->config['default_form_control_class'] = $options['form_control_class'];
        }
    ...
    function close_form() {
        //Based on the form_body variable the div's will be added automatically
        if( $this->config['form_body'] ){
            return form_close('</div></div>');
        }
            ...
            if (is_array($this->elm_options)) {
                ...
                switch ($this->func) {
                    ...
                    case 'form_html':
                        ...
                        break;
                    default:
                        //Adding block class div.
                    	if(isset($elm_options['block_class']))
                    	$this->print_string .= $this->_pre_block($elm_options['block_class']);
                        ...
                        //Closing div
                        if(isset($elm_options['block_class']))
                        $this->print_string .= $this->_post_block();
                        
                        //Adding Validation rule
                        if(isset($elm_options['rule'])){
                            $this->CI->form_validation->set_rules($elm_options['name'],isset( $elm_options['rule_label'])? $elm_options['rule_label']:$elm_options['label'],$elm_options['rule']);
                        }
...
//        }
        ...
        if ($this->func == 'form_combine') {
            ...
            switch ($this->func) {
                ...
                case 'form_dropdown':
                	...
                	/*
                	 * The Dropdown case has been modified, earlier dropdown options must be set and should not be empty
                	 * while generating a dropdown.
                	 * 
                	 * But there will be cases,  where there will be empty options in some case,  to support that !empty condition
                	 * on options dropdown has been removed.
                	 * 
                	 * Earlier Condition:  if (isset($this->elm_options['options']) && !empty($this->elm_options['options'])) {
                	 * Present Condition:  if (isset($this->elm_options['options'])) {
                	 * */
                    
                    if (isset($this->elm_options['options'])) {
                        $name = $this->elm_options['name'];
                        $options = $this->elm_options['options'];
                        $selected = isset($this->elm_options['selected']) ? $this->elm_options['selected'] : array();

                        unset($this->elm_options['name']);
                        unset($this->elm_options['selected']);
                        ...
                        $input_html_string = form_dropdown($name, $options, $selected, $this->_create_extra_string());
                    } else {
                ...
                case 'form_radio':
                    ...
                    break;
                 case 'form_info':
                    return $this->_build_info($this->elm_options);
                    break;
                default:
    ...
    private function _build_info($options) {
        $html = ' <div class="' . (! empty ( $options ['block_class'] ) ? $options ['block_class'] : 'col-md-12 col-sm-12') . '"><div class="form-group ">
                <div class="row">
                <div class="' . (! empty ( $options ['title_width'] ) ? $options ['title_width'] : 'col-md-3 col-sm-3') . '">' . (! empty ( $options ['title'] ) ? $options ['title'] : '--') . '</div>
                <div class="' . (! empty ( $options ['data_width'] ) ? $options ['data_width'] : 'col-md-9 col-sm-9') . '">' . (! empty ( $options ['data'] ) ? $options ['data'] : '--') . '</div>
                </div></div></div>';
        return $html;
    }
    ...
    private function _build_form_open($action, $attributes) {
        if($this->config['form_body']){
            return form_open_multipart($action, $attributes). '<div class="form-body white-background"><div class="row">';
        }
        return form_open_multipart($action, $attributes);
    }

    private function _pre_elm() {
        return '<div class="' . $this->config['default_input_container_class'] . ' '.@$this->elm_options['container_class'].'">';
    ...
    private function _pre_block($class) {
    	return '<div  class="' . $class . '">';
    }
    
    private function _post_block() {
    	return '</div>';
    }
    ...
    private function _pre_input() {
        ...
        if(isset($this->elm_options['control_class']))
        {
        	return '<div class="' . $this->elm_options['control_class'] . '">';
        }
        else 
        {
    ...
    private function _label() {
        ...
        if(!empty($this->elm_options['required'])) {
        	$label .= '<span class="required" aria-required="true">*</span>';
        }

        if(isset($this->elm_options['label_class']))
        {
        	$labelClassArray = array(
        			'class' => $this->elm_options['label_class']." control-label"
        	);
        }
        else
        {
        ...
        return form_label($label, $this->elm_options['name'], $labelClassArray);
    }
