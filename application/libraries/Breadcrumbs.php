...
/**
 * Breadcrumbs Class
 * Customized version by bfountain
...
class Breadcrumbs {
	 ...
	 * @return	void
	*/
	function push($page, $href,$prefix='')
	{
	// no page or href provided
		...
		// push breadcrumb
		$this->breadcrumbs[$href] = array('page' => $page, 'href' => $href,'prefix'=>$prefix);
	}
	
	// --------------------------------------------------------------------
	/**
	 * Remove crumb from stack
	 *
	 * @access	public
	 * @param	string $href
	 * @return	void
	...
	function show()
	{
		if ($this->breadcrumbs) {
			...
			foreach ($this->breadcrumbs as $key => $crumb) {
				...
				if (end($keys) == $key) {
					$output .= $this->crumb_last_open . $crumb['prefix'] . $crumb['page'] . '' . $this->crumb_close;
				} else {
					$output .= $this->crumb_open.$crumb['prefix'] .'<a href="' . $crumb['href'] . '">' . $crumb['page'] . '</a> '.$this->crumb_divider.$this->crumb_close;
				}
			}
