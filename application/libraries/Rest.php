...
class REST
{
    ...
    function setJsonArray($value=false){
    ...
    protected function _call($method, $uri, $params = array(), $format = NULL)
    {
     ...
     // echo "<pre>";
	 // print_r($this->rest_server.$uri);echo "</pre>";
    ...
    public function debug()
    {
        ...
        $data['response_code'] = $this->status();
        $data['response'] = $this->response_string;
		$data['headers'] = $this->_ci->curl->get_headers();
        $data['call_details'] = print_r($this->_ci->curl->info, true);
        
        if ($this->_ci->curl->error_string)
        {
        	$data['error_code'] =  $this->_ci->curl->error_code;
        	$data['error_message'] = $this->_ci->curl->error_string;
        }

        return $this->_ci->load->view("utilities/service-debugger",$data, true);
     ...
     * @access  public
     * @author  Phil Sturgeon
     * @updated for returning array.
     * @version 1.0
     */
    protected function _json($string)
    {
        return json_decode( trim($string),$this->json_array);
    }
